...
/**
 * Production-Ready ASTM Instrument Simulator
 * 
 * Simulates laboratory instruments for comprehensive testing of the ASTM Interface Service.
 * This refactored version provides more realistic multi-frame message transmission and
 * improved resilience to network timeouts during keep-alive exchanges.
 * 
 * Key Production Features:
 * - Realistic one-record-per-frame transmission with proper ETB/ETX handling
 * - Robust timeout handling for keep-alive protocol exchanges
 * - Comprehensive error handling and recovery
 * - Enhanced logging and debugging capabilities
 * - Support for persistent connections and idle testing
 * 
...
public class InstrumentSimulator {
    ...
    private static final DateTimeFormatter ASTM_DATETIME_FORMATTER = DateTimeFormatter.ofPattern("yyyyMMddHHmmss");
    ...
    private void showMainMenu() {
        ...
        System.out.println("7. sendMessageAndWait");
    ...
    private String buildHeaderRecord(HeaderRecord header) {
        return String.format("H|\\^&|%s||%s||||||%s|%s|%s",
                ...
                header.getDateTime() != null ? header.getDateTime().format(ASTM_DATETIME_FORMATTER) : ""); // FIXED
    }
    ...
    private String buildOrderRecord(OrderRecord order) {
        return String.format("O|%d|%s|%s|%s|%s|%s",
                ...
                order.getRequestedDateTime() != null ? order.getRequestedDateTime().format(ASTM_DATETIME_FORMATTER) : ""); // FIXED
    }
    ...
    private String buildResultRecord(ResultRecord result) {
        return String.format("R|%d|%s|%s|%s||%s||%s|%s|%s",
                ...
                result.getDateTimeTestCompleted() != null ? result.getDateTimeTestCompleted().format(ASTM_DATETIME_FORMATTER) : "", // FIXED
    ...
    private void sendAstmMessage(String message) throws IOException {
        ...
        try (Socket socket = new Socket(serverHost, serverPort)) {
            ...
            while (System.currentTimeMillis() - startTime < 45000) { // 45 second timeout for testing
                try {
    ...
    /**
     * PRODUCTION-READY: Send message frames with realistic one-record-per-frame transmission
     * 
     * This method simulates real instrument behavior by sending one ASTM record per frame,
     * properly marking intermediate frames with ETB and the final frame with ETX.
     * This is crucial for testing the server's multi-frame message reassembly logic.
     */
    private void sendMessageFrames(String message, OutputStream output, InputStream input) throws IOException {
        ...
        for (int i = 0; i < records.length; i++) {
            ...
            // CRITICAL FIX: Determine if this is the last record to be sent
            boolean isLastFrame = (i == records.length - 1);
            
            // Build the frame for this single record with proper ETB/ETX marking
            String frame = buildFrame(frameNumber, record, isLastFrame);
            ...
            // Wait for ACK with improved timeout handling
            long startTime = System.currentTimeMillis();
            boolean ackReceived = false;
            while (System.currentTimeMillis() - startTime < 15000) {
                try {
            ...
            if (!ackReceived) {
                throw new IOException("No ACK received for frame " + frameNumber + " after 15 seconds");
            }
    ...
    private String buildFrame(int frameNumber, String data, boolean isLastFrame) {
        ...
        // CRITICAL FIX: Use ETB for intermediate frames, ETX for the final frame
    ...
    /**
     * PRODUCTION-READY: Receive ASTM message with improved timeout handling
     * 
     * This method now gracefully handles socket timeouts that occur during valid
     * keep-alive exchanges, preventing false "Frame timeout" errors.
     */
    private String receiveMessage(InputStream input, OutputStream output) throws IOException {
        StringBuilder message = new StringBuilder();
        int expectedFrameNumber = 1;
        ...
        while (true) {
            // Receive one complete frame with improved error handling
            ...
            if (validateFrame(frame)) {
                int frameNumber = extractFrameNumber(frame);
                String frameData = extractFrameData(frame);
                ...
                expectedFrameNumber = (expectedFrameNumber % 7) + 1;
            } else {
    ...
    /**
     * PRODUCTION-READY: Receive a single frame with resilient timeout handling
     */
    private String receiveFrame(InputStream input) throws IOException {
        ...
        // Increased timeout to handle keep-alive exchanges gracefully
        final int RECEIVE_TIMEOUT_MS = 60000; // 60 seconds
        
        while (System.currentTimeMillis() - startTime < RECEIVE_TIMEOUT_MS) {
            try {
                if (input.available() > 0) {
                    int ch = input.read();
                    if (ch == -1) {
                ...
                // CRITICAL FIX: Handle socket timeouts gracefully
                // This is expected during keep-alive exchanges
        ...
        System.err.println("Overall timeout waiting for frame after " + RECEIVE_TIMEOUT_MS + "ms");
        return null;
    }
     ...
     */
    private int extractFrameNumber(String frame) {
    ...
    /**
     * PRODUCTION-READY: Send message and maintain persistent connection for keep-alive testing
     * 
     * This method simulates the scenario where an instrument sends a message and then
     * maintains an idle connection, allowing testing of the server's keep-alive functionality.
     */
    private void sendMessageAndWait() {
        System.out.println("\n=== Sending Sample Results and WAITING (Keep-Alive Test) ===");
        AstmMessage astmMessage = createSampleResultMessage();
        String rawMessage = buildAstmMessage(astmMessage);
        ...
        try {
            ...
            socket.setSoTimeout(30000); // 30 second socket timeout for reads
            
            sendAstmMessageOverSocket(socket, rawMessage);
            ...
            while (socket.isConnected() && !socket.isClosed()) {
                try {
                    if (input.available() > 0) {
                        ...
                        if (receivedChar == ENQ) {
                            ...
                            if (serverMessage != null) {
                                System.out.println("[INFO] Received keep-alive message from server:\n" + serverMessage);
                            } else {
                                System.out.println("[INFO] Keep-alive exchange completed (no additional message content)");
                            }
                    ...
                    System.err.println("\n[ERROR] IO error during persistent connection: " + e.getMessage());
                    break;
                }
            }

        } catch (IOException e) {
            System.err.println("\nError during persistent connection: " + e.getMessage());
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    /**
     * PRODUCTION-READY: Send ASTM message over socket with improved error handling
     */
    private void sendAstmMessageOverSocket(Socket socket, String message) throws IOException {
        OutputStream output = socket.getOutputStream();
        InputStream input = socket.getInputStream();
        ...
        if (!ackReceived) {
            System.err.println("✗ No ACK received for ENQ after 15 seconds, aborting");
            return;
        }
